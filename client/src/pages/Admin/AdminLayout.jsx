...
export default function AdminLayout() {
    ...
    return (
        <div className="min-h-screen grid grid-cols-[300px_1fr] bg-[var(--color-bg-adminpanel)]">
            {/* Sidebar */}
            <aside className="sticky top-0 h-screen border-r bg-white">
                <div className="h-full flex flex-col">
                    {/* header + menu (เลื่อนภายในได้) */}
                    <div className="px-2 grow overflow-y-auto">
                        <div className="flex flex-col justify-center h-[212px] pl-6 gap-3">
                            <img src={hhLogo} alt="logo" width={56.67} height={29.88} />
                            <div className="font-semibold text-xl text-[var(--color-h1-adminpanel)]">
                                Admin panel
                            </div>
                        </div>
                    ...
                    <div className="p-3 border-t">
                        ...
                        <button
                            onClick={logout}
                            className="flex w-full text-left px-3 py-4 rounded-md hover:bg-neutral-50 gap-2"
                        >
                            <img src={signout} alt="logout" width={24} height={24} />
                            Log out
                        </button>
                    </div>
                </div>
            ...
            {/* Content */}
            <main className="bg-[var(--color-bg-layout)] min-h-screen overflow-x-hidden">
...
function Item({ to, label, icon, end = false }) {
    return (
        ...
        >
            {icon ? <img src={icon} alt="" className="h-6 w-6 object-contain opacity-70" aria-hidden /> : null}
            <span>{label}</span>
        </NavLink>
    );
}
