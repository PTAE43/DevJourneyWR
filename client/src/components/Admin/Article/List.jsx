...
export default function ArticleList({ onCreate, onEdit }) {
    ...
    const requestDelete = async (id) => {
        setDeleting(true);
        const SLOT = "articleList:delete";
        toast.loadingIn(SLOT, "Deleting…");
        try {
            const token = (await supabase.auth.getSession()).data.session?.access_token;
            const base = import.meta.env.VITE_SERVER_URL.replace(/\/+$/, "");
            const apiBase = base.endsWith("/api") ? base : `${base}/api`;
            ...
            await toast.replaceIn(SLOT, "success", "Deleted");
            setConfirm(null);
            fetchList();
        } catch (e) {
            await toast.replaceIn(SLOT, "error", e.message || "Delete failed");
            setDeleting(false);
        }
    };
    ...
    return (
        <>
            <AdminTopBar
                ...
                actions={[
                    { label: "+ Create article", onClick: onCreate, variant: "primary" },
                ]}
            />
            <div className="mt-8 mx-14 text-base font-medium">
                <div className="flex justify-between items-center gap-2 mb-4">
                    <div>
                        <form onSubmit={onSearch} className="flex items-center gap-2">
                            ...
                            {loading && (<div className="ml-1 h-4 w-4 animate-spin rounded-full border-2 border-black border-t-transparent" />)}
                        </form>
                    </div>
                    ...
                    <div className="flex gap-4">
                        <div className="relative flex items-center">
                            <select
                                className="w-[200px] border rounded-lg pl-4 py-2 text-gray-400 appearance-none cursor-pointer
                ...
                <div className="rounded-lg border overflow-hidden bg-white">
                    <table className="w-full table-fixed">
                        ...
                        <thead className="h-[48px] bg-[#f9f8f6] text-gray-400
                            ...
                            <tr>
                                <th className="text-left font-medium p-3 pl-5">Article title</th>
                ...
                <div className="mt-4 flex justify-end gap-2">
                    <button className="px-3 py-1" disabled={page <= 1} onClick={() => setPage((p) => p - 1)}>
                        {/* ย้อนกลับ */}
                    </button>
                    {hasMore && (
                        ...
                        <>
                            <div className="flex justify-center items-center gap-2">
                                ...
                                <div className="font-medium text-red-500 whitespace-nowrap">
                                    {(confirm.title || "").length > 15
                                        ? `${confirm.title.slice(0, 15)}... ?`
                                        : (confirm?.title || "")}
                                </div>
                            </div>
